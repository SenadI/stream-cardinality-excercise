...
// The implementation has been ported from https://github.com/dustin/go-probably
const (
...
// See http://algo.inria.fr/flajolet/Publications/FlFuGaMe07.pdf for details about the algorithm.
...
func (h *HyperLogLog) Merge(from *HyperLogLog) {
	if len(h.bits) != len(from.bits) {
		log.Error("HLLs are incompatible. They must have the same basis")
		os.Exit(1)
	}
